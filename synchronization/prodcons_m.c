...
Producer(void *dummy)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		if (pthread_mutex_lock(&Mutex) < 0)  로
			perror("pthread_mutex_lock");
			pthread_exit(NULL);
		}
...
main()
{
	...
	if (pthread_cond_init(&NotFull, NULL) < 0)  { // conditional variable 초기화
		perror("pthread_cond_init");
		pthread_exit(NULL);
	}
	if (pthread_cond_init(&NotEmpty, NULL) < 0)  { // conditional variable 초기화
		perror("pthread_cond_init"); 
		pthread_exit(NULL);
	}
	...
	if (pthread_create(&tid1, NULL, (void *)Producer, (void *)NULL) < 0)  { // 스레드 생성
		perror("pthread_create");
		exit(1);
	}

	if (pthread_create(&tid2, NULL, (void *)Consumer, (void *)NULL) < 0)  { // 스레드 생성
		perror("pthread_create");   
		exit(1);
	}
